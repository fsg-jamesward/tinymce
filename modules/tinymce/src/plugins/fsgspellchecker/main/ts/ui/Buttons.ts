...
const register = (editor: Editor, pluginUrl: string, startedState: Cell<boolean>, textMatcherState: Cell<DomTextMatcher | null>, currentLanguageState: Cell<string>, lastSuggestionsState: Cell<LastSuggestion | null>): void => {
  ...
  if (languageMenuItems.length > 1) {
    editor.ui.registry.addSplitButton('fsgspellchecker', splitButtonArgs);
  } else {
    editor.ui.registry.addToggleButton('fsgspellchecker', buttonArgs);
  }

  editor.ui.registry.addToggleMenuItem('fsgspellchecker', {
    text: 'Spellcheck',
    icon: 'spell-check',
    onSetup: (menuApi) => {
